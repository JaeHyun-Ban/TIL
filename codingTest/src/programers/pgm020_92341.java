...
public class pgm020_92341 {

    // https://jongwoon.tistory.com/109?utm_source=chatgpt.com
    // >> 객체를 만들어서 활요하는데 참고하면 좋을 듯
    public int[] solution(int[] fees, String[] records)   {
        ...
        for(String rec : records){
            ...
            String carNum = parts[1]; // 차량번호
            String type = parts[2]; // 입차, 출차

            if("IN".equals(type)) { // 입차일 경우
                inTime.put(carNum, time);
            } else { // 출차
                totalTime.put(carNum, totalTime.getOrDefault(carNum, 0) + (time - inTime.get(carNum)));
                inTime.remove(carNum);// 입차기록 삭제
            }

        }

        int lastMin = toMinutes("23:59"); // 마지막 출차시간
        for(Map.Entry<String, Integer> e : inTime.entrySet()){
            String carNum = e.getKey();
            int inT = e.getValue();
            totalTime.put(carNum, totalTime.getOrDefault(carNum, 0) + (lastMin - inT)); // 마지막 출차시간 계산
        }

        // 차량번호 순서 정렬
        TreeMap<String,Integer> sorted = new TreeMap<>(totalTime);
        int[] answer = new int[sorted.size()];
        int idx = 0;

        // 정산요금을 계산
        for(int time : sorted.values()){
            int fee = baseFee; // 기본요금(180)
            if(time > baseTime) {
                // (시간 - 기본요금시간) / 단위시간 * 단위요금
    ...
    public int[] solution02(int[] fees, String[] records) {
        ...
        // fees[2] = 단위시간
        // fees[3] = 단위요금
        Map<String, Integer> carMap = new HashMap<>(); // 차량번호와 주차시간을
